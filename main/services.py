...
if __name__ == '__main__':
    logging.basicConfig(filename='Relatorio.log',filemode='a' , level=logging.INFO, format='%(asctime)s | %(process)d | %(message)s', datefmt='%d-%m-%y %H:%M:%S')
    path = os.path.join(BASE_DIR, 'ArquivoMonitorado')
    print(path)
    ev_ha = LoggingEventHandler()
    obs = Observer()
    obs.schedule(ev_ha, path, recursive=True)
    obs.start()
    try:
        while True:
            sleep(1)
...
#     def modif(self, event: FileSystemEventHandler):
